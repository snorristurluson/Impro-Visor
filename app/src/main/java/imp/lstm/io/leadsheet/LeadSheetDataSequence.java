...
public class LeadSheetDataSequence implements DataSequence {
    ...
    public LeadSheetDataSequence copy() {
        ...
        beats.stream().forEach((beat) -> {
            duplicate.beats.offer(beat.copy());
        });
        chords.stream().forEach((chord) -> {
            duplicate.chords.offer(chord.copy());
        });
        melody.stream().forEach((noteStep) -> {
            duplicate.melody.offer(noteStep.copy());
        });
