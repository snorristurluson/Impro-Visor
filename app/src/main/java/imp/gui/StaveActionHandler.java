        ...
        if (!drawing && draggingSelectionHandle) {
            if (nearestLine != OUT_OF_BOUNDS) {
                ...
                if (draggingSelectionHandleLeft) {
                    if (nearestLine <= end || selectionLocked) { // user still dragging left
                        stave.setSelectionStart(nearestLine);
                    } else { // switch directions, user dragged left handle to the right beyond the right handle
                        stave.setSelectionEnd(nearestLine);
                        stave.setSelectionStart(end);
                        draggingSelectionHandleLeft = false;
                    }
                } else { // user dragging the right handle
                    if (selectionLocked) {
                        stave.setLockedSelectionEnd(nearestLine);
                    } else if (nearestLine >= start) {  // user still dragging right
                        stave.setSelectionEnd(nearestLine);
                    } else {    // switch directions, user dragged right handle to the left beyond the left handle
                        stave.setSelectionStart(nearestLine);
