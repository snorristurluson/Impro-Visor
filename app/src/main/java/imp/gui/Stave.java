...
// all notes do. All values need to be checked.
    ...
    static int getNumBeams(int rhythmValue) {
        ...
        } else if (rhythmValue <= 15) {            // 1/32nd note,
            return 3;
        } else if (rhythmValue <= 20) {            // 1/16th note, 6-
            return 2;
        } else if (rhythmValue <= 24) {            // 1/16th note, 5-
            return 2;
        } else if (rhythmValue <= 30) {            // Even 1/16th note
            return 2;
        } else if (rhythmValue <= 40) {            // 1/8th note, 3-
            return 1;
        } else if (rhythmValue <= 45) {            // A dot 1/16th note
            return 1;
        } else if (rhythmValue <= 60) {            // Even 1/8th note
            return 1;
        } else if (rhythmValue <= 80) {            // 1/4 note, 3-
            return 1;
        } else if (rhythmValue <= 90) {            // A dot 1/8th note
            return 1;
        } else {           // Even 1/4 note
            return 0;
        }
    ...
    private Image chooseImage(int pitch, int rhythmValue, int color, boolean beamed, boolean stemUp) {
        ...
        // Is the "note" a rest?
        if (!isaNote(pitch)) {
            if (rhythmValue < 10) {            // rest too small for normal rendering
                unitImage = smallbox;
            } else if (rhythmValue <= 10) {            // 1/32nd note rest, 12-
                unitImage = demisemiquaverRest;
            } else if (rhythmValue <= 12) {            // 1/32nd note rest, 10-
                unitImage = demisemiquaverRest;
            } else if (rhythmValue <= 15) {            // 1/32nd note rest,
                unitImage = demisemiquaverRest;
            } else if (rhythmValue <= 20) {            // 1/16th note rest, 6-
                unitImage = semiquaverRest;
            } else if (rhythmValue <= 24) {            // 1/16th note rest, 5-
                unitImage = semiquaverRest;
            } else if (rhythmValue <= 30) {            // Even 1/16th note rest
                unitImage = semiquaverRest;
            } else if (rhythmValue <= 40) {            // 1/8th note rest, 3-
                unitImage = quaverRest;
            } else if (rhythmValue <= 45) {            // A dot 1/16th note rest
                unitImage = semiquaverRest;
                dottedNote = true;
            } else if (rhythmValue <= 48) {            // An eighth-note quintuplet
                unitImage = quaverRest;
            } else if (rhythmValue <= 60) {            // Even 1/8th note rest
                unitImage = quaverRest;
            } else if (rhythmValue <= 80) {            // 1/4 note rest, 3-
                unitImage = crotchetRest;
            } else if (rhythmValue <= 90) {            // A dot 1/8th note rest
                unitImage = quaverRest;
                dottedNote = true;
            } else if (rhythmValue <= 96) {            // A quarter note quintuplet
                unitImage = crotchetRest;
            } else if (rhythmValue <= 120) {           // Even 1/4 note rest
                unitImage = crotchetRest;
            } else if (rhythmValue <= 160) {           // A 1/2 note rest, 3-
                unitImage = minimRest;
            } else if (rhythmValue <= 180) {           // A dotted 1/4 note rest
                unitImage = crotchetRest;
                dottedNote = true;
            } else if (rhythmValue <= 240) {           // An even 1/2 note rest
                unitImage = minimRest;
            } else if (rhythmValue <= 360) {           // A dotted 1/2 note rest
                unitImage = minimRest;
                dottedNote = true;
            } else {           // A whole rest
                unitImage = semibreveRest;
            ...
            if (rhythmValue <= 0) {
                ...
                if (rhythmValue < 10) {        // very small note value
                    unitImage = filledNoteHead[color];
                } else if (rhythmValue <= 10) {        // 1/32nd note, 12-
                    unitImage = demisemiquaverUp[color];
                } else if (rhythmValue <= 12) {        // 1/32nd note, 10-
                    unitImage = demisemiquaverUp[color];
                } else if (rhythmValue <= 15) {        // 1/32nd note
                    unitImage = demisemiquaverUp[color];
                } else if (rhythmValue <= 20) {        // 1/16th note, 6-
                    ...
                    unitImage = semiquaverUp[color];
                } else if (rhythmValue <= 30) {        // An even 1/16th note
                    unitImage = semiquaverUp[color];
                } else if (rhythmValue <= 40) {        // 1/8th note, 3-
                    unitImage = quaverUp[color];
                } else if (rhythmValue <= 45) {        // A dotted 1/16th note
                    unitImage = semiquaverUp[color];
                    dottedNote = true;
                } else if (rhythmValue <= 48) {        // An eighth-note quint
                    unitImage = quaverUp[color];
                } else if (rhythmValue <= 60) {        // An even 1/8th note
                    unitImage = quaverUp[color];
                } else if (rhythmValue <= 80) {        // 1/4 note, 3-
                    unitImage = crotchetUp[color];
                } else if (rhythmValue <= 90) {        // A dotted 1/8th note
                    unitImage = quaverUp[color];
                    dottedNote = true;
                } else if (rhythmValue <= 96) {        // A quarter-note quint
                    unitImage = crotchetUp[color];
                } else if (rhythmValue <= 120) {       // An even 1/4 note
                    unitImage = crotchetUp[color];
                } else if (rhythmValue <= 160) {       // A 1/2 note, 3-
                    unitImage = minimUp[color];
                } else if (rhythmValue <= 180) {       // A dotted 1/4 note
                    unitImage = crotchetUp[color];
                    dottedNote = true;
                } else if (rhythmValue <= 240) {       // An even 1/2 note
                    unitImage = minimUp[color];
                } else if (rhythmValue <= 360) {       // A dotted 1/2 note
                    unitImage = minimUp[color];
                    dottedNote = true;
                } else {                               // A whole note
                ...
                if (rhythmValue < 10) {        // very small note value
                ...
                } else if (rhythmValue <= 24) {        // 1/16th note quint, 5-
                    unitImage = semiquaverDown[color];
                } else if (rhythmValue <= 30) {        // An even 1/16th note
                    unitImage = semiquaverDown[color];
                } else if (rhythmValue <= 40) {        // 1/8th note, 3-
                    ...
                    unitImage = quaverDown[color];
                    dottedNote = true;
                } else if (rhythmValue <= 96) {        // An quarter-note quint
                    unitImage = crotchetDown[color];
                } else if (rhythmValue <= 120) {       // An even 1/4 note
                    unitImage = crotchetDown[color];
                } else if (rhythmValue <= 160) {       // A 1/2 note, 3-
    ...
    private class SelectionControls
        ...
        SelectionControls() {
            TransposeUp = new SelectionButton(new ImageIcon(getClass().getResource("/graphics/one.png"))) {
            ...
            TransposeDown =
                    new SelectionButton(new ImageIcon(getClass().getResource("/graphics/one.png")));
