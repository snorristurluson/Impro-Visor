...
public class PostProcessor {
     ...
     * @param approach
     * @param target
     * @return public static String getAlternateResolution(Block approach, ChordBlock target)
    {
    ...
    public static String getJoinString(Block b, Block c) {
                ...
                if (!chords.isEmpty()) {
                    ...
                    for (ChordBlock current : chords) {
                        ...
                        if (current.isDominant()) {
                            ...
                            if (traceJoin)
                                System.out.println("domKey determined by first dominant " + current + " as: " + BrickLibrary.keyNumToName(domKey));
                            return joinLookup(domKey, baseKey);
                        } else if (current.isMinor7()) {
                            long domKey = (current.getKey() + FOURTH) % OCTAVE;
                            if (traceJoin)
                                System.out.println("domKey determined by first minor7 " + current + " as: " + BrickLibrary.keyNumToName(domKey));
                            return joinLookup(domKey, baseKey);
                        }
                    }
                }
        ...
        // c is a ChordBlock, not a Brick
        {
            ...
            if (cb.isMinor7()) {
                ...
                if (traceJoin)
                    System.out.println("domkey determined by minor7 " + cb + " as " + BrickLibrary.keyNumToName(domKey));
                return joinLookup(domKey, baseKey);
            }
            // Otherwise try to use first dominant in second brick
                ...
                domKey = cb.getKey();
                if (traceJoin)
                    System.out.println("domKey determined by first dominant " + cb + " as: " + BrickLibrary.keyNumToName(domKey));
                return joinLookup(domKey, baseKey);
            } else {
